...
def _native_tools() -> list[ToolRegistryEntry]:
    return [
        _e("pubmed_search", "Search PubMed for biomedical literature by keyword, MeSH term, or PMID. Returns titles, abstracts, authors, citation counts.", CATEGORY_LITERATURE, ToolSourceType.NATIVE, capabilities=["search", "fetch_abstract", "mesh_lookup", "citation_count"]),
        _e("semantic_scholar_search", "Search Semantic Scholar for papers, citations, references, influential citations, and author profiles.", CATEGORY_LITERATURE, ToolSourceType.NATIVE, capabilities=["search", "paper_details", "citations", "references", "author_search"]),
        _e("uniprot_search", "Search UniProt for protein sequences, annotations, domains, GO terms, disease associations, and cross-references.", CATEGORY_PROTEIN, ToolSourceType.NATIVE, capabilities=["search", "fetch_entry", "sequence", "features", "cross_references"]),
        _e("esm_predict", "Run ESM-2 protein language model for embeddings, fitness prediction, and structure prediction via ESMFold.", CATEGORY_STRUCTURE, ToolSourceType.NATIVE, capabilities=["embeddings", "fitness", "fold", "contact_map"]),
        _e("kegg_search", "Query KEGG for pathways, compounds, reactions, diseases, genes, and pathway maps.", CATEGORY_PATHWAY, ToolSourceType.NATIVE, capabilities=["search", "pathway_details", "pathway_genes", "compound_details", "reaction_details"]),
        _e("reactome_search", "Search Reactome for curated biological pathways, reactions, and interactors in human and model organisms.", CATEGORY_PATHWAY, ToolSourceType.NATIVE, capabilities=["search", "pathway_details", "pathway_participants", "pathway_diagram"]),
        _e("mygene_search", "Query MyGene.info for gene annotations, orthologs, GO terms, pathway memberships, and expression data.", CATEGORY_GENOMICS, ToolSourceType.NATIVE, capabilities=["search", "gene_info", "orthologs", "go_terms", "pathway_membership"]),
        _e("chembl_search", "Search ChEMBL for bioactive compounds, targets, assays, mechanisms of action, and drug indications.", CATEGORY_DRUG, ToolSourceType.NATIVE, capabilities=["search", "compound_details", "target_details", "assay_data", "mechanism_of_action"]),
        _e("clinicaltrials_search", "Search ClinicalTrials.gov for clinical trials by condition, intervention, sponsor, phase, and status.", CATEGORY_CLINICAL, ToolSourceType.NATIVE, capabilities=["search", "trial_details", "enrollment", "outcomes", "eligibility"]),
        _e("slack_hitl", "Send messages and files to Slack channels for human-in-the-loop communication during research.", CATEGORY_COMMUNICATION, ToolSourceType.NATIVE, capabilities=["send_message", "upload_file", "create_thread"]),
        # --- New native tools (tool expansion) ---
        _e("opentargets_search", "Query Open Targets for target-disease associations, evidence, and prioritization scores.", CATEGORY_DRUG, ToolSourceType.NATIVE, capabilities=["target_disease", "evidence_search", "drug_mechanism", "tractability"]),
        _e("clinvar_search", "Search ClinVar for clinical significance of human genomic variants.", CATEGORY_VARIANT, ToolSourceType.NATIVE, capabilities=["variant_search", "clinical_significance", "condition_links", "submission_data"]),
        _e("gtex_search", "Query GTEx for gene expression across human tissues with eQTL data.", CATEGORY_EXPRESSION, ToolSourceType.NATIVE, capabilities=["tissue_expression", "eqtl", "splicing", "sample_info"]),
        _e("gnomad_search", "Query gnomAD for population allele frequencies and variant constraint metrics.", CATEGORY_VARIANT, ToolSourceType.NATIVE, capabilities=["variant_lookup", "allele_frequency", "constraint_metrics", "structural_variants"]),
        _e("hpo_search", "Search Human Phenotype Ontology for standardized phenotype terms and gene-phenotype links.", CATEGORY_ONTOLOGY, ToolSourceType.NATIVE, capabilities=["term_search", "gene_to_phenotype", "disease_to_phenotype"]),
        _e("omim_search", "Search Online Mendelian Inheritance in Man for gene-disease relationships and phenotype descriptions.", CATEGORY_GENOMICS, ToolSourceType.NATIVE, capabilities=["search", "gene_phenotype", "allelic_variants"]),
        _e("biogrid_search", "Search BioGRID for curated protein-protein, genetic, and chemical interactions.", CATEGORY_NETWORK, ToolSourceType.NATIVE, capabilities=["search", "interactions", "chemical_associations"]),
        _e("depmap_search", "Query DepMap for cancer dependency data — gene essentiality across cancer cell lines.", CATEGORY_EXPRESSION, ToolSourceType.NATIVE, capabilities=["gene_dependency", "cell_line_info", "crispr_data", "drug_sensitivity"]),
        _e("cellxgene_search", "Search CZ CELLxGENE for single-cell RNA-seq datasets and cell type annotations.", CATEGORY_EXPRESSION, ToolSourceType.NATIVE, capabilities=["study_search", "cell_types", "gene_expression", "cluster_data"]),
        _e("string_db_search", "Query STRING for known and predicted protein-protein interactions with confidence scores.", CATEGORY_NETWORK, ToolSourceType.NATIVE, capabilities=["interaction_partners", "network", "enrichment", "confidence_scores"]),
    ]
