...
AGENTIC_SYSTEM_PROMPT_SEQQA = (
    ...
    "CRITICAL: You have PRE-BUILT helper functions already loaded in your environment.\n"
    "DO NOT write your own sequence analysis code. USE THESE FUNCTIONS:\n\n"
    "```python\n"
    "# These are ALREADY IMPORTED — just call them directly:\n"
    "find_all_orfs(dna_seq)       # Returns list of ORFs across all 6 frames, sorted by length\n"
    "                              # Each: {strand, frame, length, protein, start_pos}\n"
    "get_longest_orf(dna_seq)      # Returns the single longest ORF dict\n"
    "aa_at_position(dna_seq, pos)  # Get amino acid at position N in longest ORF (1-indexed)\n"
    "reverse_complement(seq)       # Get reverse complement\n"
    "translate_sequence(seq, frame)# Translate DNA to protein (frame=0,1,2)\n"
    "six_frame_translation(seq)    # Translate all 6 frames: {'+1': ..., '-1': ..., etc.}\n"
    "find_restriction_sites(seq)   # Find restriction enzyme cut sites\n"
    "gc_content(seq)               # GC content as fraction\n"
    "find_motif(seq, motif)        # Find motif occurrences\n"
    "```\n\n"
    "EXAMPLE for ORF questions:\n"
    "```python\n"
    "seq = 'ATGATG...'  # the DNA sequence from the question\n"
    "orfs = find_all_orfs(seq)\n"
    "print(f'Found {len(orfs)} ORFs')\n"
    "print(f'Longest ORF: {orfs[0][\"length\"]} AAs, protein: {orfs[0][\"protein\"][:30]}...')\n"
    "# For 'what AA at position N' questions:\n"
    "aa = aa_at_position(seq, 10)  # position 10, 1-indexed\n"
    "print(f'AA at position 10: {aa}')\n"
    "```\n\n"
    "EXAMPLE for reverse complement questions:\n"
    "```python\n"
    "rc = reverse_complement('ATCGATCG')\n"
    "print(f'Reverse complement: {rc}')\n"
    "```\n\n"
    "RULES:\n"
    "1. You MUST write code in <execute> tags. Do NOT answer from memory.\n"
    "2. USE THE PRE-BUILT FUNCTIONS. Do NOT rewrite them.\n"
    "3. Print all intermediate results.\n"
    "4. Compare the computed result against EACH answer choice explicitly.\n"
    "5. State your final answer as: Answer: X\n\n"
    "WORKFLOW:\n"
